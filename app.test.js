...
describe('/todo', () => {
  ...
  afterAll(async () => {
    ...
    const todoItems = await request(app).get('/todo');
    for (const todoItem of todoItems.body) {
      await request(app).delete(`/todo/${todoItem.bench_todo_id}`);
    }
...
describe('/todo/:id', () => {
  describe('GET for one item', () => {
    test('should respond with status code: 200', async () => {
      ...
      const res = await request(app).get(`/todo/${newTodo.body.bench_todo_id}`);
